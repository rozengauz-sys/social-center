  ...
  return (
    <div>
      ...
      {!q.trim() && relatives.length === 0 && (
        <div style={{ color:"#94a3b8", fontSize:13, padding:"6px 0" }}>Начните вводить имя для поиска среди клиентов базы</div>
      )}
      ...
      {relatives.length > 0 && (
        <div style={{ marginTop:8, fontSize:12, color:"#6366f1", fontWeight:600 }}>
          Выбрано: {relatives.map(id => { const c = allClients.find(x => x.id === id); return c ? `${c.lastName} ${c.firstName}` : ""; }).filter(Boolean).join(", ")}
        </div>
      )}
    </div>
  );
...
function VisitHistory({ visits=[], onChange }) {
  const addVisit = () => onChange([...visits, { id:Date.now(), date:new Date().toISOString().split("T")[0], notes:"", worker:"" }]);
...
function ClientModal({ client, allClients, onSave, onClose }) {
  const [form, setForm] = useState(client ? {aid:[],visits:[],...client} : {...emptyClient, id:"new"});
      ...
      <div style={{ background:"#fff", borderRadius:16, width:"100%", maxWidth:700, maxHeight:"92vh", overflow:"auto", boxShadow:"0 25px 60px rgba(0,0,0,0.35)" }}>
        ...
        <div style={{ padding:28, display:"flex", flexDirection:"column", gap:20 }}>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
            <label style={labelStyle}>Фамилия *<input style={inputStyle} value={form.lastName} onChange={e=>set("lastName",e.target.value)} placeholder="Иванов" /></label>
            <label style={labelStyle}>Имя *<input style={inputStyle} value={form.firstName} onChange={e=>set("firstName",e.target.value)} placeholder="Иван" /></label>
          </div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
            <label style={labelStyle}>Дата рождения<input type="date" style={inputStyle} value={form.dob} onChange={e=>set("dob",e.target.value)} /></label>
          ...
          <label style={labelStyle}>Адрес<input style={inputStyle} value={form.address} onChange={e=>set("address",e.target.value)} placeholder="ул. Ленина, 5, кв. 12" /></label>
          ...
          {allClients.filter(c=>c.id!==form.id).length > 0 && (
            <SectionToggle label="Родственники" icon="🔗" defaultOpen={true}>
              <RelativeSearch formId={form.id} relatives={form.relatives} allClients={allClients}
        ...
        <div style={{ padding:"16px 28px", borderTop:"1px solid #e2e8f0", display:"flex", justifyContent:"flex-end", gap:12, position:"sticky", bottom:0, background:"#fff" }}>
          ...
          <button disabled={saving} onClick={async () => {
            if (!form.lastName||!form.firstName) return alert("Заполните фамилию и имя");
            setSaving(true);
            await onSave(form);
            setSaving(false);
    ...
    <div style={{ background:"#fff", borderRadius:12, border:"1px solid #e2e8f0", boxShadow:"0 2px 8px rgba(0,0,0,0.06)", overflow:"hidden" }}>
      <div style={{ padding:"16px 20px", display:"flex", alignItems:"flex-start", gap:16, cursor:"pointer" }} onClick={()=>setExpanded(e=>!e)}>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          ...
          <div style={{ fontSize:13, color:"#64748b", marginTop:2 }}>{client.dob && <>Д.р.: {formatDate(client.dob)} · </>}{client.address||"Адрес не указан"}</div>
  ...
  return (
    <div style={{ minHeight:"100vh", background:"linear-gradient(135deg,#1e1b4b,#312e81,#4c1d95)", display:"flex", alignItems:"center", justifyContent:"center" }}>
      <div style={{ background:"rgba(255,255,255,0.05)", backdropFilter:"blur(20px)", border:"1px solid rgba(255,255,255,0.15)", borderRadius:20, padding:48, width:380, textAlign:"center" }}>
        ...
        <button onClick={handleReset} style={{ background:"none",border:"none",color:"#a5b4fc",fontSize:13,cursor:"pointer",textDecoration:"underline" }}>
          Забыли пароль?
        </button>
      </div>
    </div>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    supabase.from("clients").select("*").order("last_name")
      .then(({ data, error }) => {
        if (data) setClients(data.map(fromDB));
  ...
  const saveClient = async (form) => {
    ...
    if (isNew) {
      ...
      const { error } = await supabase.from("clients").update(row).eq("id", form.id);
      if (error) { alert("Ошибка сохранения: " + error.message); return; }
        ...
        <div style={{ maxWidth:900, margin:"0 auto", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:12 }}>
          <div>
            ...
            <div style={{ fontSize:13, opacity:0.8, marginTop:2 }}>
              {session.user.email} · Всего: {clients.length} · Special Needs: {clients.filter(c=>c.specialNeeds).length} ·
          ...
          <div style={{ display:"flex", gap:10 }}>
            ...
            <button onClick={handleLogout} style={{ ...btnSecondary,background:"transparent",color:"#fff",borderColor:"rgba(255,255,255,0.3)" }}>🚪 Выйти</button>
          </div>
        </div>
      </div>
